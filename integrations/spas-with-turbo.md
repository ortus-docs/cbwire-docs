---
description: >-
  Create single-page-like applications (SPAs) with little-to-no JavaScript in
  record time.
---

# SPAs with Turbo

CBWIRE has built-in support for [Turbo](https://turbo.hotwired.dev/). Turbo is an open-source library that accelerates links and form submissions by negating the need for full-page reloads. With Turbo, you get all the following included:
...
You can enable Turbo with the _enableTurbo_ [configuration setting](../configuration.md), which automatically wires up everything needed to start using Turbo. Once enabled, links and form submissions will automatically be performed in the background via AJAX instead of performing full-page reloads.&#x20;

You can alternatively perform a [manual installation](spas-with-turbo.md#manual-installation) of Turbo instead.
...
To work correctly, include the Turbo plugin below your _wireScripts()_ call in your layout.
...
{% hint style="info" %}
Note: You MUST have either the _data-turbolinks-eval="false"_ or _data-turbo-eval="false"_ attributes added to the script tag (having both won't hurt).
{% endhint %}
...
Preload links into Turbo Drive’s cache using _data-turbo-preload_.
